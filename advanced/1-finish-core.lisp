...
(use-package :lisp3dev.advanced.core :lisp3dev.mspace)


(let* ((*readtable* LISP3DEV.ADVANCED.CORE:*qi-readtable*))
  (set-dispatch-macro-character 
   #\# #\{ 
   #'lisp3dev.base:|#{-READER|))

;; (DEFUN LISP3DEV.ADVANCED.CORE:QILOAD (filename)
;;   (let ((*package* (find-package :Q))
;;         (*readtable* LISP3DEV.ADVANCED.CORE:*qi-readtable*))
;;     (LISP3DEV.ADVANCED.CORE:|load| filename)))
...
(defun qi-reader (stream &key transformer pre-execute)
  (do (c 
      ...
      (end-flag
       (let* ((raw-chars (nreverse tmp))
              ...
              (raw-parsed (let ((*<caller-package>* *package*)
                                (*package* (find-package :LISP3DEV.MSPACE))
                                (*readtable* LISP3DEV.ADVANCED.CORE:*qi-readtable*)
                                (errstr "LISP3DEV.ADVANCED.CORE: parse: failure: ~%~%~{~C~} ..."))
                            (LISP3DEV.ADVANCED.CORE::|compile| 'LISP3DEV.ADVANCED.CORE::|<st_input>| chars errstr)))
                                        ;`(progn ,@(mapcar (lambda (x) `(XI.CORE::|eval| ',x))
                                        ;                  parsed))))
              (parsed (if (LISP3DEV.ADVANCED.CORE::%need-trans-p% raw-parsed)
                        (LISP3DEV.ADVANCED.CORE::%trans% raw-parsed)
                        raw-parsed))
              (form (if pre-execute
                      (let ((|*<definition-only-p>*| t))
                        (LISP3DEV.ADVANCED.CORE::%load% parsed)
                        |*<definition-only-p>*|)
                      `(LISP3DEV.ADVANCED.CORE::%load% ',parsed))))
         ;(WARN "~D" form)
         form
    ...
    (unless (characterp (setq c (read-char stream)))
      (error "LISP3DEV.ADVANCED.CORE: unexpected end of stream"))
...
;; OBSOLETE
'(lisp3dev.base:define-lpar-backslash-reader "XI" (stream)
  (FORMAT T "\\XI is OBSOLETE~%")
  (qi-reader stream))

;; OBSOLETE
'(lisp3dev.base:define-lpar-backslash-reader "XDEF" (stream)
  (FORMAT T "\\XDEF is OBSOLETE~%")
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")"))))

;; OBSOLETE
'(lisp3dev.base:define-lpar-backslash-reader "XLAMBDA" (stream)
  (FORMAT T "\\XLAMBDA is OBSOLETE~%")
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def xi_lambda " chars ")"))))

;; OBSOLETE
'(lisp3dev.base:define-lpar-backslash-reader "XDEFUN" (stream)
  (FORMAT T "\\XDEFUN is OBSOLETE~%")
  (let ((*<LOCALLY-P>* T))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))
;; OBSOLETE
'(lisp3dev.base:define-lpar-backslash-reader "XDEFMACRO" (stream)
...
;; OBSOLETE
'(lisp3dev.base:define-lpar-backslash-reader "X" (stream)
...
(lisp3dev.base:define-lpar-backslash-reader "DEF" (stream)
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")"))))

(lisp3dev.base:define-lpar-backslash-reader "LAMBDA" (stream)
  (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def xi_lambda " chars ")"))))

(lisp3dev.base:define-lpar-backslash-reader "DEFUN" (stream)
  (let ((*<LOCALLY-LET-OP>* 'LABELS))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))

(lisp3dev.base:define-lpar-backslash-reader "DEFMACRO" (stream)
  (let ((*<LOCALLY-LET-OP>* 'LABELS)
        (*<DEF-OP>* 'DEFMACRO))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))

(lisp3dev.base:define-lpar-backslash-reader "LET" (stream)
  (let ((*<LOCALLY-LET-OP>* 'FLET)
        (*<DEF-OP>* NIL))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))

(lisp3dev.base:define-lpar-backslash-reader "LETREC" (stream)
  (let ((*<LOCALLY-LET-OP>* 'LABELS)
        (*<DEF-OP>* NIL))
    (qi-reader stream :pre-execute t :transformer (lambda (chars) (<trans> "(def " chars ")")))))
...
;;OBSOLETE
;;(lisp3dev.base:define-lpar-backslash-reader "XDECL" (stream)
;;  (qi-reader stream :transformer (lambda (chars) (<trans> "(decl " chars ")"))))

;;OBSOLETE
;;(lisp3dev.base:define-lpar-backslash-reader "X-YACC" (stream)
;;  (qi-reader stream))


;; (lisp3dev.base:define-lpar-backslash-reader "X-PROLOG" (stream)
;;   (do (c 
;;        tmp
;;        escape-flag
;;        comma-flag
...
;;                         (LISP3DEV.ADVANCED.CORE::|compile| 'LISP3DEV.ADVANCED.CORE::|<horn_clauses>| chars errstr))))
;;          (if (LISP3DEV.ADVANCED.CORE::%need-trans-p% parsed)
;;            `(LISP3DEV.ADVANCED.CORE:|s-prolog| ',(LISP3DEV.ADVANCED.CORE::%trans% parsed))
;;            `(LISP3DEV.ADVANCED.CORE:|s-prolog| ',parsed))))    
