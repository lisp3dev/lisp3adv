...
(DEFINE-COMPILER-MACRO wrap (val)
  `(IF ,val 'true 'false))
(DEFUN <unwrap-error> (x) (ERROR "LISP3DEV.ADVANCED.CORE: unwrap: ~A is not a boolean" x))
(DEFUN unwrap (q_bool)
...
(DEFUN concat (&REST seqs)
  (WHEN (NULL seqs)
    (ERROR "LISP3DEV.ADVANCED.CORE: concat: call with no arg"))
...
(DEFUN abort () (ERROR "LISP3DEV.ADVANCED.CORE: aborted by (abort)"))
